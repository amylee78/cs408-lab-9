...
class Ball extends Shape {
  ...
  class EvilCircle extends Shape{
    ...
    ctx.arc(this.x, this.y, this.size, 0.4, 1.8 * Math.PI);
     //pacman mouth 
    ctx.lineTo(this.x, this.y);
    ctx.closePath();
     ctx.fillStyle = "rgba(210, 210, 20, 0.85)";
    ctx.fill();
    ctx.stroke();

    ctx.beginPath();
    //pacman eye
    ctx.arc(this.x + this.size/5, this.y - this.size/2,this.size/12, 0, 2 * Math.PI, false);
